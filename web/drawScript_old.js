...
function normalizeAnchorPath(points, anchor, options = {}) {
  ...
  // 🔒 If we're freezing the base segment (second-point preview),
  ...
  if (window.__freezeBaseSegment && second) {
    ...
    // original “heading up” enforcement for initial build
...
function ensureAnchorHeadingUp(points, cfg = autoRoundConfig) {
  ...
  if (window.__freezeBaseSegment) return points.slice(); // 🔒 do not modify base when frozen
...
function buildPerspectiveStrip(points, bounds) {
  ...
  function localRadius(i) {
    ...
    if (area2 < 1e-3) return Infinity;              // almost straight → infinite radius
...
function renderOverlayPreview(cursorX, cursorY) {
  ...
  if (currentPath.length === 1) {
    // 🚫 first click not placed yet → straight segment; do NOT curve
    window.__freezeBaseSegment = false;
    previewPoints = [ { x: A.x, y: A.y }, { x: cursorX, y: cursorY } ];

  } else if (currentPath.length === 2) {
    // ✅ first click exists → curve ONLY from P1 to cursor; freeze base (A→P1)
    ...
    // clamp sagitta by lane half-width at midpoint of P1↔cursor
    ...
    // IMPORTANT: keep A→P1 untouched; curve is only P1→G→cursor
    previewPoints = [ { x: A.x, y: A.y }, { x: P1.x, y: P1.y }, { x: Gx, y: Gy }, { x: cursorX, y: cursorY } ];

  } else {
    // ≥ 2 clicks already → normal preview (freeze not needed)
    window.__freezeBaseSegment = false;
...
function renderPath(showStopOnly = false) {
  ...
  // This avoids the “shadow” lane and ensures only the live preview is visible.
  ...
  // We’ll always reset the live preview here; on-mousemove will re-render it.
  clearOverlayPreview();
