...
#define FIRMWARE_VERSION "v2.0.72"

#define S3_ID "MINIEXCO_S3_V1_01"


// AP credentials (global)
#define AP_PASSWORD_DEFAULT "miniexco123"   // 8..63 chars; change to your default
...
void serverStart(){
  ...
  // Report AP password state (never returns the real password)
  ...
  server.on("/set_ap_password", HTTP_POST, [](AsyncWebServerRequest* req){
    ...
    // (Optional) apply immediately if AP is up. WARNING: drops clients.
    /*
...
void setup() {
  ...
  lastWebActivityMs = millis();   //Watchdog on server disconnect
}
...
void loop() {
  ...
  if (now - _lastWdTick >= 250) {
    ...
    if (now >= WS_REBOOT_GRACE_MS) {
      // If you want to skip resets during upload/stream, keep these guards:
