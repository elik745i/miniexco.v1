...
function initGamepadSwitch() {
  ...
  // Paint current state from backend (/getsettings)
  fetch("/getsettings")
    .then(r => r.json())
    .then(s => {
      // accept either key; we'll standardize on "BluepadEnabled"
      if ("BluepadEnabled" in s) sw.checked = (s.BluepadEnabled == 1);
      else if ("GamepadEnabled" in s) sw.checked = (s.GamepadEnabled == 1);
    })
    .catch(() => { /* ignore */ });

  // Send toggle via WebSocket CSV like your SystemSounds flow
  sw.onchange = () => {
    const v = sw.checked ? 1 : 0;
    if (window.ws && typeof window.ws.send === "function") {
      // Same CSV style you use elsewhere: KEY,VALUE
      window.ws.send(`GamepadEnabled,${v}`);
    }
  };
}
