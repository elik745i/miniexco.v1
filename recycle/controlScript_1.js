...
function populateJoyDropdowns() {
  ...
  root.querySelectorAll("select").forEach(sel => {
    ...
    const OPTIONS = isButtonCentric ? JOY_ACTIONS : JOY_BUTTONS;
    OPTIONS.forEach(o => {
...
function setupJoyValidationJoy() {
  ...
  const repaint = () => {
    ...
    selects.forEach(sel => {
      ...
      const chosen = (sel.value || "").trim();          // chosen ACTION (button-centric)
      if (!chosen) return;
      if (seen[chosen]) { dup = true; if (warn) { warn.textContent = "⚠️ Duplicate"; warn.style.color = "red"; } }
      else seen[chosen] = true;
    });

    if (status) { status.textContent = dup ? "Fix duplicates first!" : ""; status.style.color = dup ? "red" : ""; }
    return !dup;
  };
