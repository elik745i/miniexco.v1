...
// Radio Browser pool (same directory HA uses)
const RB_ENDPOINTS = [
...
// Utilities
function pickRbHost(){ return RB_ENDPOINTS[Math.floor(Math.random()*RB_ENDPOINTS.length)]; }
...
async function fetchCountriesRB(){
  const path = "/json/countries"; // returns {name,iso_3166_1,...}
  for (const host of RB_ENDPOINTS){
    try{
      const r = await fetch(host+path,{cache:"no-store"});
      if(!r.ok) throw new Error(r.status);
      const all = await r.json();
      return all
        .filter(c => (c.name && c.iso_3166_1))
        .map(c => ({ code: c.iso_3166_1.toUpperCase(), name: c.name }))
...
async function fetchStationsByCountry(code, limit=100){
  const q = `/json/stations/search?countrycode=${encodeURIComponent(code)}&codec=MP3&hls=false&hidebroken=true&order=clickcount&reverse=true&limit=${limit}`;
  for (const host of RB_ENDPOINTS){
    try{
      const r = await fetch(host+q,{cache:"no-store"});
      if(!r.ok) throw new Error(r.status);
      const js = await r.json();
      return js.map(s=>({ name: s.name||s.tags||"Unnamed", url: s.url_resolved||s.url })).filter(s=>s.url);
    }catch(e){}
  }
  return []; // fail soft, we still have fallbacks
}
...
function initMediaPlayer() {
  ...
  if (filesBtn) {
	...
	// Defaults
	const DEFAULT_CC = localStorage.getItem("radioCountry") || "AZ";
	...
	(function loadCountries(){
		...
		fetchCountriesRB().then(list=>{
			...
			if (!cached.length) countrySel.value = DEFAULT_CC;
			// After we have countries, ensure stations for selected
			loadStationsFor(countrySel.value, true);
		}).catch(()=>{
